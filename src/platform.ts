import { API, DynamicPlatformPlugin, Logging, PlatformAccessory, Service, Characteristic, PlatformConfig } from 'homebridge';
...
interface PiHomeConfig extends PlatformConfig {
	name: string;
	pihomeApiUrl: string;
}
...
export class PiHomeHomebridgePlatform implements DynamicPlatformPlugin {
  ...
  async discoverDevices(config: PiHomeConfig) {
    ...
    const devices: PiHomeAccessoryContext[] = datasources.map((ds: DatasourcesResponse) => ({
      dsId: ds.Id,
      uuid: this.api.hap.uuid.generate(String(ds.Id)),
      dsName: ds.Name,
      dsType: ds.Type,
      air: {
    ...
    for (const device of devices) {
      ...
      const existingAccessory: PlatformAccessory | undefined
				= this.accessories.find(accessory => accessory.UUID === device.uuid);

      if (existingAccessory) {
        // the accessory already exists
        this.log.info('Restoring existing accessory from cache:', existingAccessory.displayName);
        existingAccessory.context.device = device;
        existingAccessory.context.url = url;
        ...
        const accessory = new this.api.platformAccessory(device.dsName, device.uuid);
  ...
  createAccessoryByType(accessory: PlatformAccessory, type: string) {
    if (type === 'CO2') {
